...
function(ri, G)
  ...
  InitDataForImageRecogNode(ri).isabsolutelyirred := true;
  ...
  AddMethod(InitDataForKernelRecogNode(ri).hints,
            ...
            2000);
  InitDataForKernelRecogNode(ri).degsplittingfield := MTX.DegreeSplittingField(m)
                                   / DegreeOverPrimeField(f);
  InitDataForKernelRecogNode(ri).biggerscalarsbas := r.inforec.bas;
  InitDataForKernelRecogNode(ri).biggerscalarsbasi := r.inforec.basi;
...
function(ri, G)
  ...
  AddMethod(InitDataForImageRecogNode(ri).hints,
...
function(ri, G)
  ...
  if MTX.IsIrreducible(m) then
      ...
      if Length(collf) = 1 then    # only one homogeneous component!
          ...
          InitDataForImageRecogNode(ri).blocksize := r.blocksize;
          InitDataForImageRecogNode(ri).generatorskronecker := kro;
          AddMethod(InitDataForImageRecogNode(ri).hints,
