...
function(ri, G)
  ...
  # Hint to the image:
  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsMatrix.Scalar, 4000);
...
#
#  # Hint to the kernel:
#  InitDataForKernelRecogNode(ri).containedinsl := true;
#  return true;
#end;
...
function(ri, G)
  ...
  if nrblocks <= 2 then   # the image is only one block
      ...
      SetHomom(ri,hom);
      AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsMatrix.Scalar, 2000);
      ...
      if nrblocks = 1 then     # no kernel:
          ...
          InitDataForKernelRecogNode(ri).blocks := ri!.blocks{[1]};
          # We have to go to BlockScalar with 1 block because the one block
          # is only a part of the whole matrix:
          AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsMatrix.BlockScalar, 2000);
  ...
  InitDataForImageRecogNode(ri).blocks := List(ri!.blocks{[middle..nrblocks]},
                               x->x - (ri!.blocks[middle][1]-1));
  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsMatrix.BlockScalar, 2000);
  ...
  findgensNmeth(ri).args[2] := 5;
  InitDataForKernelRecogNode(ri).blocks := ri!.blocks{[1..middle-1]};
  AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsMatrix.BlockScalar, 2000);
  Setimmediateverification(ri,true);
  return Success;
end);
...
function(ri,G)
  ...
  InitDataForImageRecogNode(ri).blocks := bc.blocks;
  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsMatrix.BlockLowerTriangular, 4000);
...
function(ri,G)
  ...
  InitDataForImageRecogNode(ri).blocks := ri!.blocks;
  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsMatrix.BlockDiagonal, 2000);
  findgensNmeth(ri).method := FindKernelLowerLeftPGroup;
  findgensNmeth(ri).args := [];
  AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsMatrix.LowerLeftPGroup, 2000);
  InitDataForKernelRecogNode(ri).blocks := ri!.blocks;
  return Success;
end);
...
function(ri,G)
  ...
  # Now give hints downward:
  InitDataForImageRecogNode(ri).blocks := ri!.blocks;
  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsProjective.BlocksModScalars, 2000);
  ...
  if ri!.projective then
      AddMethod(InitDataForKernelRecogNode(ri).hints,
...
#  InitDataForKernelRecogNode(ri).subdim := ri!.subdim;
#  AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsMatrix.InducedOnSubspace, 2000);
#
#  return true;
#end;
#
...
InstallGlobalFunction( FindKernelLowerLeftPGroup,
  function(ri)
    ...
    InitDataForKernelRecogNode(ri).gensNvectors := lvec;
    InitDataForKernelRecogNode(ri).gensNpivots := pivots;
    InitDataForKernelRecogNode(ri).blocks := ri!.blocks;
    InitDataForKernelRecogNode(ri).lens := lens;
    InitDataForKernelRecogNode(ri).basisOfFieldExtension := basisOfFieldExtension;
...
function(ri,G)
  ...
  if HasStoredStabilizerChain(G) then
      ...
      InitDataForKernelRecogNode(ri).StabilizerChainFromAbove := S;
      return Success;
