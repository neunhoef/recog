...
function(ri,G)
  ...
  if res = fail then
      ...
      if Length(res.orb) * Length(res.orb[1]) <> ri!.dimension then
          ...
          InitDataForKernelRecogNode(ri).t := Concatenation(res.orb);
          InitDataForKernelRecogNode(ri).blocksize := Length(res.orb[1]);
          AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsProjective.DoBaseChangeForBlocks, 2000);
...
function(ri,G)
  ...
  InitDataForImageRecogNode(ri).blocksize := ri!.blocksize;
  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsProjective.Blocks, 2000);
...
function(ri,G)
  ...
  od;
  # For the image:
  InitDataForImageRecogNode(ri).blocks := blocks;
  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsProjective.BlocksModScalars, 2000);
  # For the kernel:
  InitDataForKernelRecogNode(ri).blocks := blocks;
  AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsProjective.BlocksBackToMats, 2000);
  return Success;
end);
...
function(ri,G)
  ...
  InitDataForImageRecogNode(ri).blocks := ri!.blocks{[1..Length(ri!.blocks)-1]};
...
#  InitDataForImageRecogNode(ri).blocksize := ri!.blocksize;
#  AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsProjective.BalTreeForBlocks, 2000);
#
#  # Inform authorities that the kernel can be recognised easily:
#  InitDataForKernelRecogNode(ri).subdim := subdim;
#  InitDataForKernelRecogNode(ri).blocksize := ri!.blocksize;
#  AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsProjective.BalTreeForBlocksProjKernel, 2000);
#
