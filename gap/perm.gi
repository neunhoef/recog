...
function(ri, G)
    ...
    AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsPerm.PcgsForBlocks, 400);
    AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsPerm.BalTreeForBlocks, 200);
...
function(ri, G)
  ...
  # The order of 'o' in the homom must align with InitDataForImageRecogNode(ri).blocks below
  ...
  if nrblocks - cut > 1 then
      ...
      n := Length(upperhalf);
      InitDataForImageRecogNode(ri).blocks := List([1..n],i->[(i-1)*l+1..i*l]);
      AddMethod(InitDataForImageRecogNode(ri).hints, FindHomMethodsPerm.BalTreeForBlocks, 200);
  fi;
  if cut > 1 then
      InitDataForKernelRecogNode(ri).blocks := lowerhalf;
      AddMethod(InitDataForKernelRecogNode(ri).hints, FindHomMethodsPerm.BalTreeForBlocks, 200);
  fi;
  return Success;
end);
